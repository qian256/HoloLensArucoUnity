...
bool IPPE::solvePnP(const vector<cv::Point3f>  &objPoints,const  std::vector<cv::Point2f>&imgPoints, cv::InputArray cameraMatrix, cv::InputArray distCoeffs,cv::Mat & Rvec, cv::Mat &Tvec)
{
    float markerLength = objPoints[1].y-objPoints[0].y;
     float reprojErr1, reprojErr2;
    cv::Mat rvec2(3,1,CV_64FC1);
    cv::Mat tvec2(3,1,CV_64FC1);

    IPPE::solvePoseOfCentredSquare(markerLength, imgPoints,  cameraMatrix, distCoeffs,   Rvec, Tvec,reprojErr1,rvec2,tvec2,reprojErr2);
 cerr<<reprojErr2/reprojErr1<<endl;
...
void IPPE::IPPComputeRotations(double j00, double j01, double j10, double j11, double p, double q, OutputArray _R1, OutputArray _R2)
{
    ...
    double a00, a01, a10,a11, ata00, ata01, ata10,ata11,b00, b01, b10,b11,binv00, binv01, binv10,binv11;
    ...
    a = rtilde00*rtilde11 - rtilde01*rtilde10;
