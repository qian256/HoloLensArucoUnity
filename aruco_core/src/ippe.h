...
//This is the core header file for IPPE. Infinitesimal Plane-based Pose Estimation (IPPE) is a very fast and accurate way to compute a camera's pose from a single image of a planar object using point correspondences. This has uses in several applications, including augmented reality, 3D tracking and pose estimation with planar markers, and 3D scene understanding.
...
namespace IPPE {
//returns only best solution
bool solvePnP(const vector<cv::Point3f> &objPoints,const  std::vector<cv::Point2f> &imgPoints, cv::InputArray cameraMatrix, cv::InputArray distCoeffs,cv::Mat & Rvec,cv::Mat &  Tvec);
 ...
 *
@param squareLength the squares's length (which is also it's width) in object coordinate units (e.g. millimeters, meters, etc.)
The square is defined in object coordinates on the plane z=0 and centred at the origin. Therefore its four points in object coordinates are given by:
...
@param imagePoints Array of four corresponding image points, 1x4/4x1 2-channel. Note that the points should be ordered to correspond with points 0, 1, 2 and 3.
 ...
 */
void solvePoseOfCentredSquare(float squareLength, cv::InputArray imagePoints, cv::InputArray cameraMatrix, cv::InputArray distCoeffs,
...
@param _undistortedPoints Array of corresponding image points (undistorted and normalize), 1xN/Nx1 2-channel where N is the number of points
 */
...
@param _objectPoints Array of corresponding model points, 1xN/Nx1 3-channel where N is the number of points
@param _undistortedPoints Array of corresponding image points (undistorted and normalized), 1xN/Nx1 2-channel where N is the number of points
 ...
 *
@param _targetPts Array of four corresponding target points, 1x4/4x1 2-channel. Note that the points should be ordered to correspond with points 0, 1, 2 and 3.
